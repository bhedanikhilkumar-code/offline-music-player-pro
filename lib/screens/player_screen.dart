...
class _PlayerScreenState extends State<PlayerScreen>
    ...
    try {
      ...
      if (artwork != null) {
        ...
        if (mounted) {
          setState(() {
            _dominantColor =
                palette.dominantColor?.color ?? AppColors.surfaceDark;
  ...
  Widget _buildSongTab(
      BuildContext context, AudioProvider audio, SongModel song) {
    ...
    return Column(
      children: [
        ...
        // Square Album Art with soft shadow
        Expanded(
          flex: 4,
          child: Center(
            child: Hero(
  ...
  Widget _actionIcon(IconData icon, VoidCallback onTap, {Color? color}) {
    return IconButton(
      icon: Icon(icon, color: color ?? Colors.white70, size: 28),
      onPressed: onTap,
    );
  }
}
