            ...
            child: Column(
              children: [
                ...
                Row(
                  ...
                  children: [
                    _actionButton(context, Icons.notifications_outlined,
                        AppStrings.setAsRingtone, () async {
                      if (Navigator.canPop(context)) {
                        Navigator.pop(context);
                      }
                      final success =
                          await RingtoneService.setAsRingtone(song.path);
                      if (context.mounted) {
                    ...
                    _actionButton(
                        context, Icons.image_outlined, AppStrings.changeCover,
                        () {
                      if (Navigator.canPop(context)) {
                        Navigator.pop(context);
                      }
                    ...
                    _actionButton(
                        context, Icons.label_outline, AppStrings.editTags, () {
                      if (Navigator.canPop(context)) {
                        Navigator.pop(context);
                      }
                      Navigator.push(
                ...
                Row(
                  ...
                  children: [
                    _actionButton(
                        context, Icons.album_outlined, AppStrings.goToAlbum,
                        () {
                      if (Navigator.canPop(context)) {
                        Navigator.pop(context);
                      }
                    }),
                    _actionButton(context, Icons.visibility_off_outlined,
                        AppStrings.hideSong, () {
                ...
                // List options
                _listTile(Icons.speed_rounded, AppStrings.speed,
                    onTap: () => _showSpeedDialog(context)),
                _listTile(Icons.tune_rounded, AppStrings.playbackSettings,
                    subtitle: AppStrings.crossfadeGapless),
                Consumer<SettingsProvider>(
                  builder: (context, settings, _) => SwitchListTile(
                    secondary: const Icon(Icons.brightness_low_rounded,
                        color: Colors.white70),
                    title: Text(AppStrings.keepScreenOn,
  ...
  void _showDeleteConfirmation(BuildContext context) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Delete "${song.displayTitle}" from device?',
          ...
          TextButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                    content: Text(
                        'Delete feature requires MANAGE_EXTERNAL_STORAGE')),
              );
            },
        ...
        content: Column(
          ...
          children: [0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0].map((speed) {
            return ListTile(
              ...
              trailing: audio.speed == speed
                  ? Icon(Icons.check, color: AppColors.accentOrange)
                  : null,
              onTap: () {
