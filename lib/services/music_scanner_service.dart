...
class MusicScannerService {
    ...
    try {
      ...
      return songs
          ...
          .map((s) => SongModel(
                ...
                duration: s.duration ?? 0, path: s.data,
                dateAdded: s.dateAdded ?? 0, size: s.size,
                albumId: s.albumId,
              ))
          .toList();
    } catch (e) { return []; }
  ...
  static Future<List<SongModel>> getSongsForAlbum(int albumId) async {
    try {
      ...
      return songs
          .where((s) => s.albumId == albumId && s.duration != null && s.duration! > 0)
          .map((s) => SongModel(
                id: s.id, title: s.title,
                artist: s.artist ?? '<unknown>',
                album: s.album ?? 'Unknown Album',
  ...
  static QueryArtworkWidget getArtwork(int id, {ArtworkType type = ArtworkType.AUDIO, double size = 50}) {
    return QueryArtworkWidget(
      id: id, type: type,
      artworkHeight: size, artworkWidth: size,
